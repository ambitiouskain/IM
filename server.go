...
func (this *Server) Handler(conn net.Conn) {
	...
	//创建用户实例,获取信息
	user := NewUser(conn, this) //传入当前的server的地址针，方便user调用

	//调用用户上线业务
	user.Online()

	//监听用户是否活跃的channel
	isLive := make(chan bool) //? 创建一个channel，用于接收用户活跃信号

	// 接受客户端发送的消息
	go func() {

		//创建缓冲区buf，用于储存从用户端读取到的数据（即消息）
		buf := make([]byte, 4096)

		for {

			//从用户端的连接conn中读取数据并存储到buf中
			n, err := conn.Read(buf) //n：读取到的字节数。正式开始连接

			if n == 0 { //用户主动关闭连接，没有字节传输（无数据可以读取）
			...
			if err != nil && err != io.EOF { //读取中发生错误：忽略连接正常关闭的EOF错误，打印其他错误信息
			...
			//用户针对msg进行消息处理：将得到的消息进行广播（即显示给其他用户）
			user.DoMessage(msg)

			//根据用户是否发送了消息，来判断是否为活跃状态
			isLive <- true //? 用户每发送一条消息，都会向 isLive 通道发送true信号，表明该用户活跃
		}
	}()
	...
	for {
		...
		//当前用户是活跃的，应该重置定时器
		case <-isLive: //? 如果有数据被发送到 isLive 通道（例如：isLive <- true），此 case 会被触发。
			//当前用户活跃
			//! 不做任何处理，仅激活select从而跳出超时逻辑以重置计时器

			//用户300s没发消息，则执行超时处理，踢出用户。
		case <-time.After(time.Second * 300): //? 如果 300 秒内没有触发其他 case（例如没有从 isLive 通道收到信号），该 case 将触发。
			//已经超时
			user.SendMsg("你被踢了，请不要挂机")
